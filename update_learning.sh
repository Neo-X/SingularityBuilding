...
#
sudo singularity shell -B ~/.ssh:/root/.ssh --writable ./ubuntu_learning.img -c "pushd /opt/TerrainRLSim; git pull origin master; pushd simAdapter/; ./gen_swig.sh; popd; premake4 gmake; pushd gmake; make config=release64 -j 8; popd; popd; pushd /opt/RLSimulationEnvironments; git pull origin master; pushd rendering; premake4 gmake; make config=release64 -j 2; popd; popd;"
